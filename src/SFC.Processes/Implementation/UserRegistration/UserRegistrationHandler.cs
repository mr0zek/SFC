...
using static SFC.Processes.Contract.ConfirmUserCommand;
...
namespace SFC.Processes.Implementation.UserRegistration
{
  class UserRegistrationHandler : ICommandHandler<RegisterUserCommand>
  {
    ...
    {
      if (_query.Query<GetAccountQuery.Response, GetAccountQuery.Request>(new GetAccountQuery.Request(command.LoginName)) != null)
      {
